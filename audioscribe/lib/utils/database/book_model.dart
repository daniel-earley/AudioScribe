...
class BookModel {
  ...
  Future<List<Map<String, dynamic>>> getBookById(int bookId) async {
    ...
    final List<Map<String, dynamic>> bookMaps =
        await db.query(DbUtils.bookDb, where: 'id = ?', whereArgs: [bookId]);
  ...
  Future<List<LibrivoxBook>> getCollectionBooks() async {
    final db = await DbUtils.init();
    final List<Map<String, dynamic>> bookMaps = await db.query(DbUtils.bookDb,
        where: "isFavourite = ? OR isBookmark = ?", whereArgs: [1, 1]);

    List<LibrivoxBook> books =
        bookMaps.map((map) => LibrivoxBook.fromMap(map)).toList();
  ...
  Future<List<LibrivoxBook>> getBooksByType(String bookType) async {
    ...
    List<LibrivoxBook> books =
        bookMaps.map((bookMap) => LibrivoxBook.fromMap(bookMap)).toList();
  ...
  Future<List<Book>> getBookByType(String bookType) async {
    ...
    List<Book> books =
        bookMaps.map((bookMap) => Book.fromMap(bookMap)).toList();
